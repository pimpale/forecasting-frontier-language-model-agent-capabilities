    ...
    for i, b in enumerate(backtests):
        ...
        for split_idx in range(n_split):
            ...
            for bench_idx in range(n_bench):
                ...
                bdp: BacktestDataPoint[ObsScalingLawPredictor] = b.results[
                    split_idx, bench_idx
                ]
                bdp_copy = bdp.copy()
...
def plot_split(
    backtest: BacktestData, benchmark_id: int, x_key: str, expand=False, line=False
):
    color_list = [
        "tab:blue",
        "tab:cyan",
...
def get_split_boundaries(
    b0: BacktestData, split_key: str, benchmark_id: int, padding: float = 0.0
):
    boundaries: list[float] = []
    for i, split_ in enumerate(b0.results[:, benchmark_id]):
        split: BacktestDataPoint[ObsScalingLawPredictor] = split_
        if i == 0:
            boundaries.append(split.split_train[split_key].min())

        boundaries.append(split.split_train[split_key].max())

        if i == len(b0.results[:, benchmark_id]) - 1:
            boundaries.append(split.split_test[split_key].max())
...
def plot_capability_backtesting_figure(
    backtests: list[CapabilityBacktest], benchmark_id: int
):
    prediction_title_dict = {
        ...
        "global PC-1": "Global PC-1",
    }

    markerlist = ["o", "x", "s", "D"]

    b0 = backtests[0].backtest
    split_key = b0.splitter.key
    boundaries = np.array(
        get_split_boundaries(b0, split_key, benchmark_id, padding=0.0)
    )
    ...
    fig, ax = plt.subplots(
        1, len(backtests), figsize=(3 * len(backtests), 3.5), squeeze=False
    )
    ...
    for j, capability_backtest in enumerate(backtests):
        ...
        # first, plot the train points.
        # We need to use the final dataframe to get the ground truth data, since it will have the best-trained PC-1 score (and doesn't matter for the other models)
        bdp_g: BacktestDataPoint[ObsScalingLawPredictor] = (
            backtest.global_split_results[benchmark_id]
        )
        bdp_g_copy = bdp_g.copy()
        augment_train_test_slaw(
            bdp_g_copy.slaw,
        ...
        for split_idx in range(n_split):
            ...
            color = color_map(line_color_norm(bdp.split_train[split_key].max()))
            ...
            if extrapolatable:
                ...
                xs = np.array(
                    bdp_g_copy.split_train[f"{bdp_g.model.intermediate} pred"]
                )
                ys = np.array(bdp_g_copy2.split_train[f"{bdp.slaw.benchmark} pred"])
                ...
                lines += curr_ax.plot(
                    ...
                    ys[sort_idx],
                    label=(
                        f"Forecast predicting Split {split_idx+1}" if j == 0 else None
                    ),
                    alpha=1,
                    color=color,
                )
    ...
    fig.suptitle(
        f"Predictions for {backtests[0].backtest.results[0,benchmark_id].slaw.benchmark}"
    )
...
def plot_capability_backtesting_figure2(
    ...
    fig.suptitle(
        f"Calibration for {backtests[0].backtest.results[0,benchmark_id].slaw.benchmark}"
    )
...
def plot_capability_backtesting_figure2_hist(
    ...
    for backtest in backtests:
        ...
        method_errors.append(
            np.sqrt(test_err[:, benchmark_id])
        )  # Get RMSE for specific benchmark
        method_names.append(
            backtest.backtest.model_class.__name__.replace("Predictor", "")
        )
...
def plot_capability_backtesting_figure3(
    ...
    prediction_title_dict = {
        "LinearPC1Predictor": "Linear PC-1",
        "LogitPC1Predictor": "Logit PC-1",
        "DirectEloPredictor": "Elo",
        "DirectLogFlopPredictor": "log-FLOP (1E21)",
        "release_date": "Release Date",
        "global PC-1": "Global PC-1",
    }
    ...
    formatter = (
        year_formatter if split_key == "release_date" else lambda x, _: f"{x:.1f}"
    )
    ...
    for j, capability_backtest in enumerate(backtests):
        ...
        plotted_points = set()
        last_max_v = (
            backtest.global_split_results[benchmark_id].split_train[split_key].min()
        )
        
        splits = list(backtest.results[:, benchmark_id])
        
        for split_idx, result in enumerate(splits):
            result: BacktestDataPoint[ObsScalingLawPredictor] = result
            train = result.split_train.copy()
            test = result.split_test.copy()
            ...
            max_v = train[split_key].max()
            min_v = last_max_v
            last_max_v = max_v

            df = train[~train[split_key].isin(plotted_points)]

            y_key = f"{result.slaw.benchmark}"
            true_y = df[y_key]
            pred_y = df[f"{y_key} pred"]
            ...
            scatter = curr_ax_calib.scatter(
                ...
                pred_y,
                label=(
                    f"{formatter(min_v, None)} to {formatter(max_v, None)} (Split {split_idx})"
                    if j == 0
                    else None
                ),
                c=df[split_key],
            ...
            plotted_points.update(train[split_key])

            # Handle last split's remaining points
            if split_idx == len(splits) - 1:
                df = test[~test[split_key].isin(plotted_points)]
                ...
                curr_ax_calib.scatter(
                    ...
                    pred_y,
                    label=(
                        f"{formatter(max_v, None)}+ (Split {split_idx + 1})"
                        if j == 0
                        else None
                    ),
                    c=df[split_key],
        ...
        curr_ax_calib.set_title(
            prediction_title_dict.get(
                backtest.model_class.__name__, backtest.model_class.__name__
            )
        )
        if j == 0:
        ...
        max_values = [
            result.split_train[split_key].max()
...
def insert_global_pc1(
    data: pd.DataFrame,
    benchmarks: list[str],
) -> pd.DataFrame:
    # train linear pc1 on the data
    model_scores = torch.tensor(data[benchmarks].values, dtype=torch.float32)

    # create model
    model = LinearPC1Predictor(
        benchmarks,
        benchmark_floors=[benchmark_floor_dict[b] for b in benchmarks],
        train_model_scores=model_scores,
    )
    print("fitting model")
    model.fit()

    # insert train model scores into the data
    return data.assign(
        **{
            "global PC-1": model.predict_capability_scores_from_model_scores(
                model_scores
            )
        }
    )
...
def plot_ranked_visualization(
    data: pd.DataFrame,
    benchmarks: list[str],
):
    # Calculate number of rows and columns needed
    n_cols = min(4, len(benchmarks))
    n_rows = (len(benchmarks) + n_cols - 1) // n_cols  # Ceiling division
    ...
    for i, benchmark in enumerate(benchmarks):
        row = i // n_cols
        col = i % n_cols
        ax[row, col].scatter(data["global PC-1"], data[benchmark], label=benchmark)
        ax[row, col].set_xlabel("PC-1")
        ax[row, col].set_ylabel(benchmark)
        ax[row, col].legend()

    # Hide empty subplots
    for i in range(len(benchmarks), n_rows * n_cols):
        row = i // n_cols
        col = i % n_cols
...
base_llm_benchmark_eval = insert_global_pc1(
    base_llm_benchmark_eval, base_llm_benchmarks
)
...
# %%
ewbs_log_data = backtest_models_metric(
    ewbs, LogitPC1Predictor, base_llm_benchmark_eval, base_llm_benchmarks
)
ewbs_log_train_err, ewbs_log_test_err = compute_test_train_error(ewbs_log_data.results)
...
ewbs_date_data = backtest_models_metric(
    ewbs, DirectDatePredictor, base_llm_benchmark_eval, base_llm_benchmarks
)
ewbs_date_train_err, ewbs_date_test_err = compute_test_train_error(
    ewbs_date_data.results
)
...
print(f"Logit PC1 -> Downstream Train MSE: {ewbs_log_train_err.mean():.3f}")
print(f"Logit PC1 -> Downstream Test MSE: {ewbs_log_test_err.mean():.3f}")
...
print(f"Logit PC1 -> Downstream Train RMSE: {ewbs_log_train_err.mean()**0.5:.3f}")
print(f"Logit PC1 -> Downstream Test RMSE: {ewbs_log_test_err.mean()**0.5:.3f}")
...
# %%
plot_split(ewbs_log_data, 0, "Elo", expand=False, line=True)
...
plot_test_errmatrix_single("logit PC-1", ewbs_log_data)
plot_test_errmatrix_single("linear PC-1", ewbs_lin_data)
...
plot_capability_backtesting_figure2(
    [
        ...
        CapabilityBacktest2(ewbs_log_data),
...
plot_capability_backtesting_figure(
    [
        ...
        CapabilityBacktest(ewbs_log_data, False),
...
plot_ranked_visualization(base_llm_benchmark_eval, base_llm_benchmarks + ["TruthfulQA"])
